...
const denormalizeTags = async () => {
  ...
  while (processedProspects < totalProspectsCount) {
    ...
    const tags = await TagModel(goulagDatabase).find({ _id: { $in: tagIds }}).lean().exec();
    ...
    console.log(`${processedProspects}/${totalProspectsCount} processed users (${Math.round((processedProspects / totalProspectsCount) * 100 * 100) / 100}%)`);
  }
